<?php
 ...
 //variable para la cantidad filas por la paginacion
    ...
    //hacemos la consulta para que aparezcan solo los proyectos del coordinador que inicio sesion

    $stmt = $conexion->prepare("SELECT * FROM proyectos WHERE idCoord = :nombreUsuario");
                    ...
                    <tbody>
                        ...
                        if (count($result) > 0){
                            foreach ($result as $fila){
                                ...
                                echo '<form action="detalles/detallesProyectoCo.php" method="POST">';
                                echo '<input type="hidden" name="codigo" id="codigo" value="'.$fila['idProyecto'].'"/>';
                                echo '<button type="submit" class="btn btn-dark btn-sm text-light" style="margin-top: 10px;">Editar</button>';
                                echo '</form>';
                                echo '<form action="tablas/tablaTareasProyectos.php" method="POST">';
                                echo '<input type="hidden" name="codigo" id="codigo" value="'.$fila['idProyecto'].'"/>';
                                echo '<button type="submit" class="btn btn-dark btn-sm text-light" style="margin-top: 10px;">VerTareas</button>';
                                echo '</form>';
                                echo '<form action="eliminar/eliminarProyectosCo.php" method="POST">';
                ...
                if (isset($_GET['pagina'])){
                        if ($_GET['pagina'] > 1) {
                            echo '<li class="page-item">
                                    <a class="page-link" href="indexCoordinador.php?pagina='. ($_GET['pagina'] - 1) .'" arial-label="Previous>
                      ...
                      for ($i=1; $i <=$cantidadPaginas; $i++){
                        if ($_GET['pagina'] == $i) {
                            ...
                            echo '<li class="page-item disabled"><a class="page-link" href="indexCoordinador.php?pagina='.$i.'">'.$i.'</a></li>';
                        } else {
                            echo '<li class="page-item"><a class="page-link" href="indexCoordinador.php?pagina='.$i.'">'.$i.'</a></li>';;
                        }
                        ...
                        if ((($_GET['pagina']) * $productosPorPagina) < $maximoElementos) {
                            echo '<li class="page-item">
                                    <a class="page-link" href="indexCoordinador.php?pagina='. ($_GET['pagina'] + 1) .'" aria-label="Next">
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content bg-dark text-white">
                                    ...
                                    <div class="modal-body">
                                        <form action="registrar/registrarProyectoCoor.php" method="POST">
                                            <div class="mb-3">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" id="idCoord" name="idCoord" value="<?php echo $nombreUsuario?>" readonly require>
                                            </div>
